...
public:
  explicit peer_filter(const QString& filter_file)
  {
    ...
    while (ifs >> peer_id >> client) {
      ...
      QString msg_tmpl(u"'%1': invalid %2 matching expression '%3' detected at line %4, ignoring rule"_qs);
      int line = m_filters.size() + 1;

      if (!peer_id_re.isValid())
        LogMsg(msg_tmpl.arg(log_tag).arg(u"peer id").arg(peer_id_re.pattern()).arg(line), Log::WARNING);

      if (!client_re.isValid())
        LogMsg(msg_tmpl.arg(log_tag).arg(u"client name").arg(client_re.pattern()).arg(line), Log::WARNING);
